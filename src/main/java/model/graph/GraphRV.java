...
public class GraphRV {
    ...
    public GraphRV(Set<User> listWaitingUsers, Set<Vehicle> listVehicles, int vehicleCapacity, int maxEdgesRV, int maxEdgesRR, Environment env, int currentTime) {
        this.env = env;
...
//    }
    ...
    private EdgeRV getRREdgesFromRequests(User request1, User request2) {
        ...
        for (Node[] validSequence : sequencesPkDpTwoRequests) {
            PathBuilder draftVisit = Visit.getDraftVisit(validSequence, this.currentTime);
    ...
    private EdgeRV createEdgeRV(User request, Vehicle vehicle) {
        ...
        while (perms.hasNext()) {
            ...
            PathBuilder draftVisit = env.getDraftVisit(vehicle, perms.next());
