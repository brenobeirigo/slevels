...
public class VisitRelocation extends Visit {
     ...
     */
    public VisitRelocation(Node targetNode, Vehicle vehicle, Environment env) {
        // Create a target node for rebalancing
        NodeTargetRebalancing target = new NodeTargetRebalancing(vehicle, targetNode, env);
...
//            this.departure = vehicle.getMiddleNode().getEarliestDeparture();
            distOM = env.getNetwork().getDistSec(vehicle.getLastVisitedNode().getNetworkId(), start.getNetworkId());
        }
        ...
        List<Integer> path = env.getNetwork()
...
//        System.out.println(subset);
        ...
        for (int i = 0; i < path.size() - 2; i++) {
            dep = dep + env.getNetwork().getDistSec(path.get(i), path.get(i + 1));
...
//            this.arrivals.get(dep + Dao.getInstance().getDistSec(this.path.get(i), this.path.get(i + 1)));
        ...
        this.idle = 0;
        this.delay = distOM + env.getNetwork().getDistSec(start.getNetworkId(), target.getNetworkId());
        this.delayBonus = 0;

        // Arrival at target node (latest time at vehicle current node + distance to target)
        this.targetArrival = vehicle.getEarliestDeparture() + env.getNetwork().getDistSec(vehicle.getLastVisitedNode().getNetworkId(), target.getNetworkId());
    }
